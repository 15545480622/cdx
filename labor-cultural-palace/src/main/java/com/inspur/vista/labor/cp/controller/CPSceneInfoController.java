...
/**
 * @Title: CPSceneDefinitionController
 * @Description: 预约场次定义，根据预约计划自动生成控制器
 * @Author: gengpeng
 * @CreateDate: 2020/03/06 09:28
 * @Version: 1.0
 */
@Api(value = "场次定义接口", tags = {"场次定义接口"})
@RestController
@RequestMapping("/api/cp/scene/info")
public class CPSceneInfoController {

    private static final Logger logger = LoggerFactory.getLogger(CPSceneInfoController.class);
    ...
    public ResponseDTO<CPSceneInfoVO> listCPSceneDefinition(@ApiParam(name = "parameters", value = "查询条件，courtId：场地id（必填）;cxBeginDate：开始时间（到天）；cxEndDate结束时间（到天）", required = true) @RequestBody Map<String, Object> parameters) {
        ...
        try {
            // 不排除过期的场次
            parameters.put("fileterExpireScene", false);
